...
func timeFromExcelTime(excelTime float64, date1904 bool) time.Time {
	...
	if intPart <= 61 {
		...
		datePart := float64(intPart)     // 日期部分
		timePart := excelTime - datePart // 纯时间部分
		// 先转换日期部分
		var date time.Time
		if date1904 {
			date = julianDateToGregorianTime(MJD_0+OFFSET1904, datePart)
		} else {
			date = julianDateToGregorianTime(MJD_0+OFFSET1900, datePart)
		}
		// 再精确添加时间部分
		timeSeconds := timePart * 86400
		duration := time.Duration(timeSeconds * float64(time.Second))
